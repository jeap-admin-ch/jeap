...
| Name | Mandatory | Default | Description | Example |
| --- | --- | --- | --- | --- |
| `jeap.messaging.authentication.subscriber.require-signature` | N | `false` | Strict mode on or off: if `true`, every message except whitelisted ones must have a signature. If `false`, signatures are not checked as long as the headers `jeap-sign`, `jeap-sign-key`, `jeap-cert` are set. | `false` |
