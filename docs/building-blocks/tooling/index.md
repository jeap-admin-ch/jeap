...
| jeap-central-publishing-maven-plugin | Fork of Sonatype's `central-publishing-maven-plugin` that patches HTTP client creation to respect HTTP proxy system properties. Used to publish jEAP artifacts to Maven Central from behind a proxy. | [GitHub](https://github.com/jeap-admin-ch/jeap-central-publishing-maven-plugin) |
